...
**职责**：多模型协作的角色编排、分工调度与验收闭环

**分层结构**：
- **编排层**：Supervisor/Planner/Reviewer，负责拆解、分配、验收
- **执行层**：Frontend/Backend/Test/Docs 等，面向产出

**调度流程**：
1. Planner 产出任务树 + 验收标准
2. 分工 Agent 并行执行并回写结果
3. Reviewer 评审质量与风险
4. Supervisor 决策合并或回滚
...
**配置示例**（`configs/agents.yaml`）：
```yaml
roles:
  planner:
    model: openai:gpt-4o
    tools: [read_file, summarize, plan]
  reviewer:
    model: openai:gpt-4o
    tools: [read_file, diff, review]
  frontend:
    model: gemini:1.5-pro
    tools: [read_file, write_file, image_generate]
  backend:
